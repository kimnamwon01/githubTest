...
3. 주요 특징
	1) 다양한 데이터 유형 지원 :
		oracle sql은 표준 숫자, 문자열 데이터 유형 외에도 날짜, 타임스탬프, xml 등
		다양한 데이터 유형을 지원합니다.
	2) 고급 sql 기능 : 분석함수, 모델 함수, 정규표현식 등 고급 기능을 제공하여 복잡한
		데이터 분석과 처리가 가능합니다.
4. sql과 일반 프로그램 언어의 차이점
	1) sql
		용도 : 데이터베이스에서 ㅔㄷ이터를 추출하여 문제 해결
		입출력 : 입력은 테이블, 출력은 테이블
		번역 : DBMS
...
from emp : 테이블명
where 조건문 : 컬럼명을 비교/논리연산식에 의해서 filtering 처리 시 사용한다.
...
-- 4) 4개 컬럼명 순서를 바꾸어 출력..
SELECT ename, job, mgr, empno FROM emp;
SELECT DEPTNO FROM emp;
-- distinct : 중복된 데이터를 제거하고 보이게 할 때.
SELECT DISTINCT deptno
FROM emp;
-- ex) emp 테이블에 각 열별로 중복되는 컬럼을 확인하고, distinct로 중복을
--		제거하고 열별로 출력해보세요..
SELECT * FROM emp;
SELECT DISTINCT job FROM emp;
SELECT DISTINCT mgr FROM emp;
SELECT DISTINCT deptno, job -- 두 개 컬럼 모두를 기준으로 중복 안되게 출력..
FROM emp;
/*
# 컬럼명 별칭으로 사용하기
...
2. 그러나, 특정한 경우에는 컬럼명을 변경하거나 통합된 컬럼명으로 처리해야 하는 경우가 있다.
	...
	select empno||ename 사원번호와 사원명연결
			ename || '님' 사원명과 '님' 문자연결
	# 데이터베이스에서는 + 연산자는 숫자형 연산시만 활용, 문자열 연결 시 문자열||문자열 활용한다.
3. 기본형식
	select 컬럼명 as 컬럼명 별칭 : as를 키워드로 별칭 사용
		   컬럼명 컬럼명별칭 : 공백을 이용해서 별칭 사용
4. 별칭컬럼명의 예외 사항
	1) 컬럼명은 특수 문자나 공백이 포함된 경우
		"특수문자"
		와 같이 "" 사이에 넣어서 처리한다.
 * */
SELECT *
FROM emp; -- 초기에 컬럼명과 해당 구조로 테이블을 만들었기에 그 기본 순서를 출력해준다.
-- 컬럼명과 컬럼순서, 타입과 구성요소 변경.. ==> alias명, ||, 연산자 활용하여 처리..
...
-- 다음의 각 컬럼에 alias를 한글로 설정하여 출력하세요 as, 공백 둘 다 활용
-- empno : 사원번호, ename 사원명, job 직 책, mgr 관리자 번호, hiredate 입사일
-- sal : $급여$, comm 보너스, deptno 부서번호, 'Dear!' || ename 호칭 으로 처리하여 출력하세요..
SELECT empno 사원번호, ename 사원명, job "직책", mgr "관리자 번호", hiredate 입사일,

...
3. 아래와 같이 3가지 영역으로 나누어서 java코드와 html 코드들을 혼합해서 사용된다.
4. 종류
	1) 선언부(declaration)
		<%!
		...
		java의 클래스로 비유하면 class 바로 밑에 선언된 부분이다. (필드, 생성자, 메서드)
	2) scriptlet
		스크립트의 조각이라는 말로, 실제 실행되는 자바코드가 들어가는 부분이다.
		<%
		%>
		자바로 비유하면 main()선언된 부분, 로직처리 등 화면을 위해 모든 기반되는 
		처리를 하는 코드를 말한다. 
	3) expression(표현식)
		화면구성 <%=자바변수%>

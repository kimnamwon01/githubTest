...
3. servlet의 주요 작업은.
	java + html 혼합된 코드를 html 코드로 변환하여 브라우저에 stream 방식으로 전송
4. client 브라우저에서는 서버로부터 받은 html코드를 해석하여 화면에 출력
5. 그 이후로 js 코드의 여러 가지 기능(이벤트 핸들러 등)을 처리한다.

# 내장 객체
1. jsp에서는 servlet으로 처리되게 기본적으로 내장 객체를 선언하고 사용하고 있다.
2. 즉, 변경된 servlet에서 매개변수로 받는 객체 뿐만 아니라 여러가지 선언되어 바로 사용할 수
	있는 내장 객체가 있다.
	protected void service(HttpServletRequest request, 
							HttpServletResponse response) throws ServletException, IOException {
	1) servlet으로 변환되었을 때, 메개변수로 위와 같이, 
